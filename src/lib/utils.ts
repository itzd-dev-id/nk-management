...
export function getBadgeColor(groupName: string, isSelected?: boolean): string {
    ...
    // Structure & Civil
    if (name.includes('footplat')) return 'bg-blue-100 text-blue-600 border-blue-200';
    if (name.includes('tie beam')) return 'bg-cyan-100 text-cyan-600 border-cyan-200';
    if (name.includes('kolom')) return 'bg-violet-100 text-violet-600 border-violet-200';
    if (name.includes('balok') || name.includes('pelat')) return 'bg-emerald-100 text-emerald-600 border-emerald-200';
    if (name.includes('tanah') || name.includes('galian')) return 'bg-amber-100 text-amber-600 border-amber-200';

    // Architecture
    if (name.includes('dinding')) return 'bg-rose-100 text-rose-600 border-rose-200';
    if (name.includes('lantai')) return 'bg-orange-100 text-orange-600 border-orange-200';
    if (name.includes('plafon')) return 'bg-sky-100 text-sky-600 border-sky-200';
    if (name.includes('pintu') || name.includes('jendela')) return 'bg-teal-100 text-teal-600 border-teal-200';

    // MEP
    if (name.includes('listrik')) return 'bg-yellow-100 text-yellow-600 border-yellow-200';
    if (name.includes('air') || name.includes('plumbing')) return 'bg-indigo-100 text-indigo-600 border-indigo-200';

    // Default / Logistics
    if (name.includes('pengiriman') || name.includes('logistik')) return 'bg-slate-100 text-slate-600 border-slate-200';
    if (name.includes('smkk') || name.includes('keselamatan')) return 'bg-red-100 text-red-600 border-red-200';
